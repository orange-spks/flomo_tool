...
// let helpReadState = {
//     isActive: false,
//     mainContentBackup: null,
//     mainContainer: null
// };
//
// function getHelpReadMainContainer() {
...
// }
//
// const HELP_READ_NOISE = /^(点赞|在看|分享|收藏|热门|搜索|公众号|小程序|上一篇|下一篇|相关文章|返回|原创|阅读|发表|评论)\s*$/i;
//
// function extractArticleToTxt(rawText, pageTitle) {
//     if (!rawText || !rawText.trim()) return (pageTitle || '').trim();
...
//         if (p.length < 5) return !HELP_READ_NOISE.test(p);
//         return true;
//     });
//     const body = filtered.join('\n\n').replace(/\n{3,}/g, '\n\n').trim();
//     const out = (pageTitle ? pageTitle + '\n\n' : '') + body;
//     return out.slice(0, 8000);
// }
//
// function getHelpReadArticleText() {
//     const container = getHelpReadMainContainer();
//     const pageTitle = (document.title || '').trim();
//     const raw = (container === document.body)
//         ? (document.body.innerText || document.body.textContent || '')
//         : (container.innerText || container.textContent || '');
//     return extractArticleToTxt(raw, pageTitle);
// }

// function fetchHelpReadAnalysis(articleText, streamResults = false) { ... }

// const HELP_READ_MAX_CHARS = 2000;
// function splitArticleIntoChunks(articleText) { ... }

// function injectHelpReadStyles() { ... }
// function removeHelpReadStyles() { ... }

// function buildHelpReadDOM(articleAnalysis) { ... }
// function applyHelpRead(articleAnalysis) { ... }
// function restoreHelpRead() { ... }
// function updateHelpReadButtonLabel(btn) { ... }

// ---------- 辅助阅读进度显示和流式渲染（已注释停用） ----------
// function createHelpReadProgress() { ... }
// function updateHelpReadProgress(progress, percentage, status, detail) { ... }
// function removeHelpReadProgress() { ... }

// function createStreamingResultContainer(totalChunks) { ... }

// function updateStreamingChunk(chunkIndex, chunkData) { ... }
// chrome.runtime.onMessage.addListener for helpReadProgress { ... }
